$PBExportHeader$w_nomina_org.srw
forward
global type w_nomina_org from w_center
end type
type pb_anular from picturebutton within w_nomina_org
end type
type pb_cancelar from picturebutton within w_nomina_org
end type
type pb_2 from picturebutton within w_nomina_org
end type
type cb_1 from commandbutton within w_nomina_org
end type
type dw_aus from datawindow within w_nomina_org
end type
type cbx_1 from checkbox within w_nomina_org
end type
type hpb_1 from hprogressbar within w_nomina_org
end type
type dw_apemp from datawindow within w_nomina_org
end type
type sle_fila from singlelineedit within w_nomina_org
end type
type tab_1 from tab within w_nomina_org
end type
...
type dw_tsup from datawindow within w_nomina_org
end type
type dw_1 from datawindow within w_nomina_org
end type
type dw_tipoincap from datawindow within w_nomina_org
end type
type dw_empact from datawindow within w_nomina_org
end type
type tab_n from tab within w_nomina_org
end type
...
type st_15 from statictext within w_nomina_org
end type
type mle_1 from multilineedit within w_nomina_org
end type
type sle_4 from singlelineedit within w_nomina_org
end type
type pb_calcula from picturebutton within w_nomina_org
end type
type dw_historia_bak from datawindow within w_nomina_org
end type
type dw_ausend from datawindow within w_nomina_org
end type
type st_14 from statictext within w_nomina_org
end type
type st_16 from statictext within w_nomina_org
end type
type pb_pila from picturebutton within w_nomina_org
end type
type dw_nelec from uo_datawindow within w_nomina_org
end type
type lb_1 from listbox within w_nomina_org
end type
type dw_pila from datawindow within w_nomina_org
end type
end forward
...
global w_nomina_org w_nomina_org
...
string is_path="c:\windows\temp\",l_tnom,l_cesan,ls_vac,ls_vpriprev, ls_vpagdo0
boolean calculando, cancelar
uo_datastore dw_historia , ids_req , ids_novedad
...
public function integer recalcula ()
...
dw_concep_cargo.DataObject = "dw_conceptos_cargo"
...
for k = 1 to dw_historia.RowCount()
	...
	for i =  1 to dw_concep_cargo.RowCount()	
		...
		if	l_tnom='V' and dw_concep_cargo.GetItemString(i,'tipo')='0' and dw_concep_cargo.GetItemString(i,'novedad')='0' then continue
		if	l_tnom='V' and dw_concep_cargo.GetItemString(i,'tipo')='P' then continue
		if	l_tnom='V' and dw_concep_cargo.GetItemString(i,'tipo')='1' then continue
...
public function integer recalcula ();long i, filahist, h,ll_fila
decimal cantidad
string tipo,msg
...
next
if get_values(tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'tipodoc'),tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'documento')) = -1 then Return -1
calctotal()
Return 0
...
if calculando then
	...
	for i = 1 to dw_aus.RowCount()
		...
		if ls_vac='1' then 
			...
			if month(date(dw_aus.GetItemDate(i,'fechaini'))) = mes and year(date(dw_aus.GetItemDate(i,'fechaini'))) = ano and calculando  then
	...
	if tipo_nomina<>'V' then 
		...
		for l_j = 1 to dw_tipoincap.RowCount()
			...
			for i = 1 to dw_aus.RowCount()
				...
				if (not isNull(iret.x)) and (not isNull(iret.y)) then
					...
					if  inc_t > 0 and calculando  then
						ciclo=1
		...
		for i = 1 to dw_aus.RowCount()
			...
			l_luto= l_luto+ inc_t
			if l_luto > 0 and calculando then
				filan = tab_1.p_2.dw_novedad.Find("sigla='"+dw_aus.GetItemString(1,'sigla')+"'",1,tab_1.p_2.dw_novedad.RowCount())
				if filan = 0 then
					filan= tab_1.p_2.pb_addnov.Event clicked()
					tab_1.p_2.dw_novedad.SetItem(filan,'nsalario',dw_historia.getitemnumber(filahist,'nsalario'))
...
public function integer nomcalc (string tipodoc, string documento, string pesp, string ptnom);st_interfec iper, icar, iret // Intervalos de fechas
long fila, j, i, h, aus, dias, filahist, fcontra
string des_concep, valor, tipo
...
if dw_historia.Retrieve(tipodoc, documento,pesp,ptnom) > 0 then
	f_retrieveEnd()
	filahist = dw_historia.Find("diasvinculado > 0",1,dw_historia.RowCount())
...
blob bfc,bfc1
string maxitem,cod_tipo, sigla, tipo_con, form_calculo,form_verifica,ls_codp
int li_k=1
uo_datastore dw_prestaact

dw_prestaact = create uo_datastore
dw_prestaact.DataObject = "dw_prestaact"
dw_prestaact.SetTransObject(SQLCA)

if tab_n.tpn_1.dw_nomcab.GetItemString(tab_n.tpn_1.dw_nomcab.GetRow(),'tipo')<>'R' and tab_n.tpn_1.dw_nomcab.GetItemString(tab_n.tpn_1.dw_nomcab.GetRow(),'tipo')<>'V' then
	if dw_prestaact.Retrieve(tab_n.tpn_1.dw_nomcab.GetItemDateTime(tab_n.tpn_1.dw_nomcab.GetRow(),'inicia'), tab_n.tpn_1.dw_nomcab.GetItemDateTime(tab_n.tpn_1.dw_nomcab.GetRow(),'termina'),tipodoc, documento) > 0 then
		...
		for j = 1 to dw_prestaact.RowCount()
			...
			select sigla,cod_tipo_concep,tipo,des_concep,tarifa,tarifa_esp into :sigla,:cod_tipo,:tipo_con,:des_concep,:tarifa,:tarifa_esp
			from nom_concep where cod_concep = :cod_conceppresta;
			...
			tab_1.p_2.dw_novedad.SetItem(fila,'tipodoc',tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'tipodoc'))
			tab_1.p_2.dw_novedad.SetItem(fila,'documento', tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'documento'))
			tab_1.p_2.dw_novedad.SetItem(fila,'cod_concep',cod_conceppresta)
			tab_1.p_2.dw_novedad.SetItem(fila,'item',j)
			tab_1.p_2.dw_novedad.SetItem(fila,'cod_tipo_concep',cod_tipo)
			tab_1.p_2.dw_novedad.SetItem(fila,'des_concep',des_concep)
			tab_1.p_2.dw_novedad.SetItem(fila,'sigla',sigla)
			tab_1.p_2.dw_novedad.SetItem(fila,'tipo',tipo_con)
			...
			if wf_vac_anticipada() = '1' then
				...
				if  vac=0 then 
					diasVacSig=f_dias_vac(tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'tipodoc'), tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'documento'),ls_vac, tab_n.tpn_1.dw_nomcab.GetItemDateTime(tab_n.tpn_1.dw_nomcab.GetRow(),'inicia') , tab_n.tpn_1.dw_nomcab.GetItemDateTime(tab_n.tpn_1.dw_nomcab.GetRow(),'termina')  ) 
				else
					diasVacSig=0
				end if
				diasVacAct = f_dias_vac_per(tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'tipodoc'), tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'documento'), tab_n.tpn_1.dw_nomcab.GetItemDateTime(tab_n.tpn_1.dw_nomcab.GetRow(),'inicia')  ) 
			...
			////se agrega para caluclos ese indias
			if f_addcompute(sigla, string(round(dw_prestaact.GetItemNumber(j,'valor')+ vrVac,0)),string(round(dw_prestaact.GetItemNumber(j,'valor')+ vrVac,0)),tarifa,tarifa_esp,li_k) = -1 then Return -1	
		next
	end if
	...
	if dw_ahorroact.Retrieve(tipodoc, documento) > 0 then
		...
		for j = 1 to dw_ahorroact.RowCount()		
			...
			if fila = 0 then
				...
				fila = tab_1.p_2.dw_novedad.Find("sigla='"+ sigla +"'",1,tab_1.p_2.dw_novedad.RowCount())
...
////se agrega para calculos ese indias						
					if f_addcompute(sigla, string(round(dw_ahorroact.GetItemNumber(j,'vfijo') +vrVac,0)),string(round(dw_ahorroact.GetItemNumber(j,'vfijo')+vrVac,0)),tarifa,tarifa_esp,li_k) = -1 then Return -1
				else
					...
					if f_addcompute(sigla, form_calculo,form_verifica,tarifa,tarifa_esp,li_k) = -1 then Return -1
					long k
					dw_historia.SetItem(filahist, f_get_ctrl(sigla),1)
...
this.Control[iCurrent+5]=this.dw_aus
this.Control[iCurrent+6]=this.cbx_1
this.Control[iCurrent+7]=this.hpb_1
this.Control[iCurrent+8]=this.dw_apemp
this.Control[iCurrent+9]=this.sle_fila
this.Control[iCurrent+10]=this.tab_1
this.Control[iCurrent+11]=this.dw_tsup
this.Control[iCurrent+12]=this.dw_1
this.Control[iCurrent+13]=this.dw_tipoincap
this.Control[iCurrent+14]=this.dw_empact
this.Control[iCurrent+15]=this.tab_n
this.Control[iCurrent+16]=this.st_15
this.Control[iCurrent+17]=this.mle_1
this.Control[iCurrent+18]=this.sle_4
this.Control[iCurrent+19]=this.pb_calcula
this.Control[iCurrent+20]=this.dw_historia_bak
this.Control[iCurrent+21]=this.dw_ausend
this.Control[iCurrent+22]=this.st_14
this.Control[iCurrent+23]=this.st_16
this.Control[iCurrent+24]=this.pb_pila
this.Control[iCurrent+25]=this.dw_nelec
this.Control[iCurrent+26]=this.lb_1
this.Control[iCurrent+27]=this.dw_pila
end on

on w_nomina_org.destroy
call super::destroy
...
type pb_grabar from w_center`pb_grabar within w_nomina_org
integer x = 3968
integer y = 24
...
string title = "none"
string dataobject = "dw_clase_ter"
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
boolean livescroll = true
...
if this.GetColumnName() = 'sigla'  or lower(dwo.name)= 'sigla' then
	...
	if dwc_novedad.GetItemString(dwc_novedad.GetRow(),'tercero')='2' then
		dw_apemp.retrieve(tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'tipodoc'),tab_n.tpn_2.dw_empnom.GetItemString(tab_n.tpn_2.dw_empnom.GetRow(),'documento'))
		dw_apemp.SetFilter("cod_clase='"+dwc_novedad.GetItemString(dwc_novedad.GetRow(),'cod_tipo_concep')+"'")
		dw_apemp.Filter()
		...
		if GetItemString(GetRow(),'tipo') <> 'L' then 
			...
			for i = 1 to RowCount()
				...
				if dw_historia.Describe(GetItemString(i,'sigla') + '.Coltype') = "!" then
					//messagebox("verificar",'Error formula '+GetItemString(i,'sigla'))
...
boolean enabled = false
string title = "none"
string dataobject = "dw_tsup_resumen"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
openwithparm(w_tipo_nomina,ltipon)
ltipon = Message.stringParm
if ltipon='!' then return -1
...
fila = dw_nomcab.InsertRow(1)
dw_nomcab.setItem(fila,'num_nomina', numdoc)
dw_nomcab.setItem(fila,'ano', year( today() ))
dw_nomcab.setItem(fila,'mes', month( today() ))
dw_nomcab.setItem(fila,'tipo',ltipon)
dw_nomcab.setItem(fila,'fecha', today() )
dw_nomcab.setItem(fila,'periodo', '5')
dw_nomcab.setItem(fila,'inicia', date("1/" + string(today(),"mm/yyyy")))
dw_nomcab.setItem(fila,'termina', RelativeDate(date(inicMesSig(datetime(today()))), -1))
dw_nomcab.setItem(fila,'estado', '0')
dw_nomcab.setItem(fila,'contabil', 'P')
dw_nomcab.setItem(fila,'lugar', clugar)
il_nuevo = fila
dw_nomcab.scrolltoRow(fila)
...
If l_tnom='C' then 
	Select cod_concep,des_concep,sigla into :l_cesan,:desco,:l_sigla from nom_concep Where (((nom_concep.cesantia)='1'));
	if isnull(l_cesan) then 
	...
	for l_c=1 to dw_empact.rowcount()		
		...
		fila=tab_1.p_2.pb_addnov.TriggerEvent(clicked!)
		tab_1.p_2.dw_novedad.SetItem(fila,'cod_concep',l_cesan)		
		tab_1.p_2.dw_novedad.SetItem(fila,'des_concep',desco)	
		tab_1.p_2.dw_novedad.SetItem(fila,'sigla',l_sigla)
		tab_1.p_2.dw_novedad.AcceptText ( )
		tab_1.p_2.dw_novedad.event itemchanged(fila,tab_1.p_2.dw_novedad.object.sigla,tab_1.p_2.dw_novedad.getitemstring(fila,'sigla'))
	next
	pb_calcula.TriggerEvent(clicked!)
	tab_n.tpn_2.dw_empnom.SetRedraw(true)
	tab_1.p_1.dw_devenga.SetRedraw(true)
...
If l_tnom='V' then 
	...
	setNull(desco)
	setNull(l_sigla)
	Select cod_concep,des_concep,sigla into :l_cesan,:desco,:l_sigla from nom_concep Where (((vac)='1') and estado='1');
	if isnull(l_cesan) then 
...
If l_tnom='P' then 
	...
	tab_1.p_2.rb_amb.checked=false
	tab_1.SelectTab(2)
	openwithparm(w_nomina_prima,l_cesan)
	l_cesan= Message.stringParm
	if l_cesan='!' then return -1
	Select cod_concep,des_concep,sigla into :l_cesan,:desco,:l_sigla from nom_concep Where ((cod_concep=:l_cesan) and estado='1');
	for l_c=1 to dw_empact.rowcount()		
		tab_n.tpn_2.dw_empnom.ScrolltoRow(l_c)
		fila=tab_1.p_2.pb_addnov.TriggerEvent(clicked!)
		tab_1.p_2.dw_novedad.SetItem(fila,'cod_concep',l_cesan)	
