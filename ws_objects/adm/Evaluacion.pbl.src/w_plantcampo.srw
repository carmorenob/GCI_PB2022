...
int f,n
f = dw_c.GetRow()
if dw_c.GetItemString(f,'conducta') = '1' then
...
elseif dw_c.GetItemString(f,'conducta') = '0' then
	if dw_c.GetItemString(f,'codtingre') = '1' then
	...
	if (dw_c.GetItemString(f,'codtingre') = '2' or dw_c.GetItemString(f,'codtingre') = '3' or dw_c.GetItemString(f,'codtingre') = '4') and (dw_c.GetItemString(f,'ingreso') = '0' or dw_c.GetItemString(f,'qingreso') = '0') then
	...
	if (dw_c.GetItemString(f,'codtingre') = '2' or dw_c.GetItemString(f,'codtingre') = '3' or dw_c.GetItemString(f,'codtingre') = '4') and (dw_c.GetItemString(f,'ingreso') = '1' or dw_c.GetItemString(f,'qingreso') = '1') then
...
selectblob plantilla into :memo from hisplantillacampo
...
if not isnull(texto) and texto<>blob('') then
	sqlca.autocommit=true
	updateblob hisplantillacampo set plantilla=:texto 
	where codplantilla=:cp and numcampo=:nc;
