...
for j=1 to dw_liqgrupo.rowcount()
	...
	if dw_liqgrupo.getitemstring(j,"tiposervicio")="M" then
		dw_liqgrupo.setitem(j,"valor",vunit*st_c.puntos*porcent*bilate)
	else
		dw_liqgrupo.setitem(j,"valor",vunit*porcent*bilate)
	end if
next
	...
	case "edita"
		...
		if st_c.tiposerv="Q" then
			...
			for j=1 to st_c.dw_factura_cpo.rowcount()
				...
				if cual<>0 then dw_liqgrupo.setitem(cual,"escog","1")
			next
