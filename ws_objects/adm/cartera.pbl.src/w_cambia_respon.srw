...
event doubleclicked;string emp,cont,tingres,es_medica, tafil, estrato,tip_liq,plan
int edad
long j,filaf
...
estrato = w_simula_fac.dw_emppac.getitemstring(w_simula_fac.dw_emppac.GetRow(),"estrato")
select codta into :tafil from emppac where tipodoc=:w_simula_fac.td and documento=:w_simula_fac.doc and codemp=:emp and codcontrato=:cont;
if sqlca.sqlcode=-1 then
	messagebox("Error leyendo emppac",sqlca.sqlerrtext)
end if
//edad=w_principal.dw_1.getitemnumber(1,"anyos")
edad =  round(w_simula_fac.f_edad_dias(w_simula_fac.dw_emppac.GetItemDateTime(w_simula_fac.dw_emppac.GetRow(),'fnacimiento'))/365,0)
...
for j=1 to dw_deta.rowcount()
	...
	if es_medica='0' then
		...
		select valor into :valoru from tarifa_medica where codmanual=:manual and c_medica=:proceq;
	end if
	if sqlca.sqlcode=-1 then
