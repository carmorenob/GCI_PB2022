...
for j=1 to dw_liqgrupo.rowcount()
	...
	if cbx_bilateral.checked then
		cuanto=f_aproxima_a(vunit*porcent*(1+dw_liqgrupo.getitemnumber(j,"bilateral")/100),100)
		dw_liqgrupo.setitem(j,"valor",cuanto)
	else
		dw_liqgrupo.setitem(j,"valor",f_aproxima_a(vunit*porcent,100))
	end if
next
...
string l_tip_liq
select ctlqx into :l_tip_liq from manuales where codmanual=:st_c.manual;
for k=1 to dw_liqgrupo.rowcount()
	if l_tip_liq='1' then 
		dw_liqgrupo.setitem(k,"vunit",f_aproxima_a(dw_liqgrupo.getitemnumber(k,"vunit")*pormanual,100))
	else
	...
	case "edita"
		...
		if st_c.tiposerv="Q" then
			...
			for j=1 to st_c.dw_factura_cpo.rowcount()
				...
				if cual<>0 then 
					dw_liqgrupo.setitem(cual,"esco","1")
