$PBExportHeader$w_print_histor_org.srw
forward
global type w_print_histor_org from window
end type
type pb_3 from picturebutton within w_print_histor_org
end type
type rte_1 from richtextedit within w_print_histor_org
end type
type rb_2 from radiobutton within w_print_histor_org
end type
type rb_comp from radiobutton within w_print_histor_org
end type
type ddl_tipo from dropdownlistbox within w_print_histor_org
end type
type gb_1 from groupbox within w_print_histor_org
end type
type pb_1 from picturebutton within w_print_histor_org
end type
type pb_2 from picturebutton within w_print_histor_org
end type
type dw_2 from datawindow within w_print_histor_org
end type
type dw_1 from datawindow within w_print_histor_org
end type
type dw_3 from datawindow within w_print_histor_org
end type
end forward
...
string title = "Imprimir Historia Clínica Agruapada"
...
global w_print_histor_org w_print_histor_org
...
blob  detalle, notas
string clug,l_temporal="C:\windows\temp\"

if isvalid(w_reporte_gral) then close(w_reporte_gral)
l_temporal=l_temporal+tipdoc+docu+".jpg"
...
if rb_comp.checked then //completa
	...
	for j=1 to dw_1.rowcount()
		if dw_1.getitemstring(j,'sel_plant')='1' then
			...
			if st.p_contador<>dw_1.getitemnumber(j,'contador') then //ingreso nuevo colocar el agrupamiento de ese ingreso
			...
			if i_rep.retrieve(dw_1.getitemnumber(j,'contador'),dw_1.getitemstring(j,'clugar'),dw_1.getitemstring(j,'codplantilla'),'0')>0 then
				st.p_ds=i_rep
				st.p_clug=dw_1.getitemstring(j,'clugar')
				st.p_cod_plant=dw_1.getitemstring(j,'codplantilla')
				st.p_titulo='~b~r'+string(dw_1.getitemdatetime(j,'fecha'),'dd/mm/yyyy HH:mm')+': '+upper(dw_1.getitemstring(j,'desplantilla'))+'~b'
		...
		choose case dw_2.getitemstring(j,'cual')
			...
			case else
				...
				if i_rep.retrieve(dw_2.getitemnumber(j,'contador'),dw_2.getitemstring(j,'clugar'),dw_2.getitemstring(j,'codplantilla'),'0')>0 then
					...
					st.p_titulo='~r'+string(dw_2.getitemdatetime(j,'fecha'),'dd/mm/yyyy hh:mm a/p')+': '+upper(dw_2.getitemstring(j,'desplantilla'))+'~r'
					if isvalid(w_reporte_gral) then
						w_reporte_gral.pasar_a_rtf(st)
					else
						opensheetwithparm(w_reporte_gral,st,w_principal,7,original!)
