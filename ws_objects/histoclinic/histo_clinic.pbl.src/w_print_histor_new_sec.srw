$PBExportHeader$w_print_histor_new_sec.srw
forward
global type w_print_histor_new_sec from window
end type
type rb_3 from radiobutton within w_print_histor_new_sec
end type
type cbx_1 from checkbox within w_print_histor_new_sec
end type
type st_1 from statictext within w_print_histor_new_sec
end type
type ddlb_archivos from dropdownlistbox within w_print_histor_new_sec
end type
type dw_3 from datawindow within w_print_histor_new_sec
end type
type pb_3 from picturebutton within w_print_histor_new_sec
end type
type rb_2 from radiobutton within w_print_histor_new_sec
end type
type rb_comp from radiobutton within w_print_histor_new_sec
end type
type ddl_tipo from dropdownlistbox within w_print_histor_new_sec
end type
type gb_1 from groupbox within w_print_histor_new_sec
end type
type pb_1 from picturebutton within w_print_histor_new_sec
end type
type pb_2 from picturebutton within w_print_histor_new_sec
end type
type rte_1 from richtextedit within w_print_histor_new_sec
end type
type barra from hprogressbar within w_print_histor_new_sec
end type
type dw_1 from datawindow within w_print_histor_new_sec
end type
type dw_2 from datawindow within w_print_histor_new_sec
end type
end forward
...
string title = "Imprimir Historia Clínica Secuencial"
...
global w_print_histor_new_sec w_print_histor_new_sec

type variables
trae i_historial
constant string union1="  SELECT historial.contador,historial.clugar,historial.indapdx,hclin_registro.fecha_reg as fecha,'1'AS sel_ing,case when hclin_registro.tipo='F' then '0' else '1'  end  AS sel_plant,hclin_registro.codplantilla,hclin_registro.desplantilla,hclin_plant.ayuda,hclin_plant.reporte,hclin_registro.ingreso ,hclin_registro.tipo,hclin_registro.nregistro,historial.fecha as hf     FROM (historial INNER JOIN hclin_registro ON (historial.CONTADOR = hclin_registro.CONTADOR) AND (historial.CLUGAR = hclin_registro.CLUGAR)) INNER JOIN hclin_plant ON hclin_registro.CODPLANTILLA = hclin_plant.CODPLANTILLA  "
...
string ped_ing,paci,ls_impr,l_ruta,ls_path='C:\windows\temp\',l_tipo
...
public subroutine f_imprime_rte (string p_sql_todo, string p_sql_historia)
public subroutine f_imprime_ole (string p_sql_todo, string p_sql_historia)
end prototypes
...
for l_h=1 to p_rep.rowcount()
	if ((p_rep.getitemnumber(l_h,'nregistro')<> nreg) or (p_rep.getitemnumber(l_h,'nregistro')=nreg and l_h=1)) then
		if (l_h=1 and p_cp=1)  or (p_contador<>dw_1.getitemnumber(p_cp,'contador') )then
			if p_epi='C' then
		...
		p_oles.Selection.TypeText("~n"+up_enc+"~n")
		p_oles.Selection.Font.Size = 9
		p_oles.selection.ParagraphFormat.Alignment = 3
		up_enc=string(p_rep.getitemdatetime(l_h,'fecha_reg'),'dd/mm/yyyy HH:mm:ss')+': '+upper(p_rep.getitemstring(l_h,'desprof'))
							...
							case '1','3','4'
								if len(result_s.getitemstring(c_r,'resultado'+result_s.getitemstring(c_r,'tipocampo')))<>0 then 
									 ...
									 if not isnull(up_res) and up_res<>'-1' then
										 ...
										 if result_s.getitemstring(c_r,'tipocampo')='3' then 
	...
	if nreg<>p_rep.getitemnumber(l_h,'nregistro')	 then 
		p_oles.ActiveDocument.saveas(ls_impr, 17)
		st_1.text='Guardo '+string(nreg)
	end if	
next
p_oles.ActiveDocument.saveas(ls_impr, 17)
end subroutine

public subroutine f_imprime_rte (string p_sql_todo, string p_sql_historia);long l_j
blob notas,detalle
uo_datastore result_s
oleobject oles,abre
datetime ffinal
...
if rb_comp.checked then //completa
	...
	for l_j=1 to dw_1.rowcount()
		if dw_1.getitemstring(l_j,'sel_plant')='1' then
			...
			if st.p_contador<>dw_1.getitemnumber(l_j,'contador') /* or  l_j=1 */ then //ingreso nuevo colocar el agrupamiento de ese ingreso
			...
			p_sql_todo=p_sql_historia+"((hclin_registro.Contador="+string(dw_1.getitemnumber(l_j,'contador')) +") AND (hclin_reg_valor.clugar='"+dw_1.getitemstring(l_j,'clugar')+"') AND (hclin_registro.nregistro="+string(dw_1.getitemnumber(l_j,'nregistro'))
	...
	st.p_header=lf_encab_agrupa('E',0,'','')
	for l_j=1 to dw_2.rowcount()
		if dw_2.getitemstring(l_j,'sel_plant')='0' then continue
		choose case dw_2.getitemstring(l_j,'cual')
			case 'NQX'
				i_rep.dataobject='dr_ingresoqx'
				i_rep.settransobject(sqlca)
				if st.p_contador<>dw_2.getitemnumber(l_j,'contador') then //ingreso nuevo colocar el agrupamiento de ese ingreso
					st.p_encab_ingreso=lf_encab_agrupa('A',dw_2.getitemnumber(l_j,'contador'),dw_2.getitemstring(l_j,'clugar'),dw_2.getitemstring(l_j,'indapdx'))
					st.p_contador=dw_2.getitemnumber(l_j,'contador')
					st.p_clug=dw_2.getitemstring(l_j,'clugar_his')					
				else
					st.p_encab_ingreso=''
				end if

				i_rep.retrieve(dw_2.getitemnumber(l_j,'numero_ingre'),dw_2.getitemstring(l_j,'clugar_his'))
				st.p_ds=i_rep
				sqlca.autocommit=true
				selectblob  deta_proc_qx  into  :detalle from lugar where codlugar=:st.p_clug;
				if isnull(detalle) then detalle=blob('')
				selectblob notas_qx  into :notas from lugar where codlugar=:st.p_clug;
				sqlca.autocommit=false
			...
			case 'EPI'
				...
				st.p_contador=dw_2.getitemnumber(l_j,'contador')
				if i_rep.retrieve(dw_2.getitemnumber(l_j,'contador'),dw_2.getitemstring(l_j,'clugar'))>0 then
					st.p_ds=i_rep
					st.p_clug=dw_2.getitemstring(l_j,'clugar')
					st.p_cod_plant='EPI'
			...
			case else
				...
				st.p_contador=dw_2.getitemnumber(l_j,'contador')
				if i_rep.retrieve(dw_2.getitemnumber(l_j,'contador'),dw_2.getitemstring(l_j,'clugar'),dw_2.getitemstring(l_j,'codplantilla'),'0')>0 then
					st.p_ds=i_rep
					st.p_clug=dw_2.getitemstring(l_j,'clugar')
					st.p_cod_plant=dw_2.getitemstring(l_j,'codplantilla')
					st.p_titulo='~r'+string(dw_2.getitemdatetime(l_j,'fecha'),'dd/mm/yyyy HH:mm')+': '+upper(dw_2.getitemstring(l_j,'desplantilla'))+'~r'
					if isvalid(w_reporte_gral) then
						w_reporte_gral.pasar_a_rtf(st)
					else
						opensheetwithparm(w_reporte_gral,st,w_principal,7,original!)
...
public subroutine f_imprime_ole (string p_sql_todo, string p_sql_historia);oleobject oles,abre
datetime ffinal
string up_txt,up_enc,ls_enc,ldt,l_qx
int wdHeaderFooterPrimary = 1
long c_p,c_h,c_r,l_j
blob notas
st_xa_print_histo st
boolean paso
	...
	if rb_comp.checked then //completa
		...
		for  c_p=1 to dw_1.rowcount()
			if dw_1.getitemstring(c_p,'sel_plant')='1' then
				...
				i_rep.settransobject(sqlca)
				p_sql_todo=p_sql_historia+"((hclin_registro.Contador="+string(dw_1.getitemnumber(c_p,'contador')) +") AND (hclin_reg_valor.clugar='"+dw_1.getitemstring(c_p,'clugar')+"') AND (hclin_registro.nregistro="+string(dw_1.getitemnumber(c_p,'nregistro'))
				p_sql_todo+="))"+"order by HISTORIAL.INDAPDX,hclin_registro.fecha,case when hclin_registro.tipo='H' then  case when hclin_registro.ingreso ='I' then  0  else  10 end else case when hclin_registro.tipo='E' then 1 else case when hclin_registro.tipo='R' then 2 else case when hclin_registro.tipo='Q' then 3 else case when hclin_registro.tipo='F' then 4 else case when hclin_registro.tipo='L' then 5 else case when hclin_registro.tipo='P' then 6  end end end end end end end, hclin_reg_valor.orden"
				if i_rep.setsqlselect(p_sql_todo)=-1 then
					p_sql_todo=i_rep.describe('DataWindow.Table.Select')
				end if
				if i_rep.retrieve()>0 then
					f_impresion(oles,i_rep,dw_1.getitemnumber(c_p,'contador'),dw_1.getitemstring(c_p,'clugar'),c_p,'C')
				end if
			end if
			barra.position=c_p
		Next
		...
		for c_p=1 to dw_2.rowcount()
			...
			//if (round((c_p/300),0)*300)=c_p then 
...
on w_print_histor_new_sec.create
...
on w_print_histor_new_sec.destroy
destroy(this.rb_3)
destroy(this.cbx_1)
destroy(this.st_1)
...
choose case text
	case 'El Actual Ingreso'
		sql_tod=union1+' where '+paci+ped_ing//+union2+paci+ped_ing
		sql_res=un1_res+' and '+paci+ped_ing + un2_res+' and ' +paci+ped_ing + un3_res+' where' +paci+ped_ing
		l_tipo='1'
	case 'Toda la Historia'
		sql_tod=union1+' where '+paci//+union2+paci
...
event clicked;long c_p,l_contador,l_q
string sql_historia,sql_todo,l_temporal,l_qx,l_clug
string recorre
uo_datastore result_s
...
//verificar si es de consulta externa y unico
if i_historial.tingres='1' then
	If l_tipo='1'  then
		f_imprime_rte(sql_todo,sql_historia)
	end if
	If l_tipo='2' then
		...
		if c_p>=25 then
			f_imprime_ole(sql_todo,sql_historia)
		else
			f_imprime_rte(sql_todo,sql_historia)	
		end if
		setnull(c_p)
