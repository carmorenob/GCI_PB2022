$PBExportHeader$w_estad_ria.srw
forward
...
type cb_cancel from picturebutton within w_estad_ria
end type
type cb_guarda from picturebutton within w_estad_ria
end type
...
event open;st_rip=message.powerobjectparm
dw_rias.retrieve(st_rip.contador,st_rip.clugar_his)
...
event clicked;if dw_rias.update(true,false) = -1 then 
	rollback;
else
	commit;
	dw_rias.resetupdate()
...
choose case colum
	case 13, 15, 17, 33, 35, 37
				...
				if i<>row and not isnull(this.getitemstring(i,"r_"+mid(dwo.name,3))) and this.getitemstring(i,"r_"+mid(dwo.name,3))='1' then										
					if isnull(this.getitemstring(i,string(dwo.name))) then
						...
						if  (colum =13 or colum=15 or colum=17) then 
							veri=f_valida_fin_con(this.getitemstring(i,'s_fin_consulta'),this.getitemstring(i,'s_causaexterna'),w_principal.dw_1.getitemstring(1,'sexo'),w_principal.dw_1.getitemnumber(1,'dias'),this.getitemstring(i,'s_diagprin'))
							if veri=-1 then
		...
		if  (colum =13 or colum=15 or colum=17) then 
			veri=f_valida_fin_con(this.getitemstring(row,'s_fin_consulta'),this.getitemstring(row,'s_causaexterna'),w_principal.dw_1.getitemstring(1,'sexo'),w_principal.dw_1.getitemnumber(1,'dias'),this.getitemstring(row,'s_diagprin'))
			if veri=-1 then
