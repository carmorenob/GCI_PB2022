$PBExportHeader$w_asig_cita_new.srw
forward
global type w_asig_cita_new from window
end type
type dw_ver from datawindow within w_asig_cita_new
end type
type dw_3 from datawindow within w_asig_cita_new
end type
type pb_1 from pb_report within w_asig_cita_new
end type
type cb_guarda from picturebutton within w_asig_cita_new
end type
type cb_2 from picturebutton within w_asig_cita_new
end type
type cb_lleva from picturebutton within w_asig_cita_new
end type
type cb_guarda_yrec from picturebutton within w_asig_cita_new
end type
type pb_2 from picturebutton within w_asig_cita_new
end type
type pb_nocita from picturebutton within w_asig_cita_new
end type
type fecha_esperada from editmask within w_asig_cita_new
end type
type st_6 from statictext within w_asig_cita_new
end type
type rte_1 from richtextedit within w_asig_cita_new
end type
type st_4 from statictext within w_asig_cita_new
end type
type dw_2 from datawindow within w_asig_cita_new
end type
type em_3 from editmask within w_asig_cita_new
end type
type pb_print from pb_report within w_asig_cita_new
end type
type tab_2 from tab within w_asig_cita_new
end type
...
type tab_1 from tab within w_asig_cita_new
end type
...
type st_consul from statictext within w_asig_cita_new
end type
type em_2 from editmask within w_asig_cita_new
end type
type em_1 from editmask within w_asig_cita_new
end type
type dw_1 from datawindow within w_asig_cita_new
end type
type dw_cruce from datawindow within w_asig_cita_new
end type
type dw_agenda from datawindow within w_asig_cita_new
end type
type mc_1 from monthcalendar within w_asig_cita_new
end type
type dw_escog_profe from datawindow within w_asig_cita_new
end type
type cbx_1 from checkbox within w_asig_cita_new
end type
type st_cita from statictext within w_asig_cita_new
end type
type dw_cuenta_citas from datawindow within w_asig_cita_new
end type
type dw_cita_asig from datawindow within w_asig_cita_new
end type
end forward
...
global w_asig_cita_new w_asig_cita_new
...
if isnull(dw_cita_asig.getitemnumber(1,"ncita")) then
	...
	dw_cita_asig.setitem(1,'cod_fina',dw_escog_profe.getitemstring(1,'cod_fina'))	
	datetime fec
...
if  tab_1.tp_1.dw_serv_cita.getitemnumber(l_i,'esc')=1 then
	if getitemstring( l_i,"sel"+cual)="1" then 
		...
		if i_cuantos < tab_1.tp_1.dw_serv_cita.getitemnumber(l_i,"nturnos")*tab_1.tp_1.dw_serv_cita.getitemnumber(l_i,"cantidad") then
			...
			donde=tab_1.tp_2.dw_serv_turno.find(pa_buscar+' and date(fecha)=date("'+string(fecha,'yyyy-mm-dd')+'")',1,tab_1.tp_2.dw_serv_turno.rowcount())
			if donde<>0 then
...
boolean bringtotop = true
string title = "none"
string dataobject = "dw_escog_prof_consul_new"
boolean border = false
boolean livescroll = true
end type

event itemchanged;string fil_fina,ls_sex, filtro
double li_dias
...
choose case getcolumn()
	case 1
		...
		if isnull(getitemstring(1,1)) then
			dw_escog_profe.setfilter("codgc=''")
		else
			fil_fina=" indsexo in('0','"+ls_sex+"') and  "+string(li_dias)+">=edadini  and  "+string(li_dias)+"<=edadfin and codgc='"+getitemstring(1,1)+"'"
			dw_escog_profe.setfilter("codgc='"+getitemstring(1,1)+"'")
		end if
		if not isnull(i_dw_grupc.getitemstring(i_dw_grupc.getrow(),"codaadx")) then
			Modify("cod_fina.dddw.name='dw_finconsulta'")	
			fil_fina=''
		else
			Modify("cod_fina.dddw.name='dw_finconsulta_citas'")			
		end if	
		dw_escog_profe.getchild("cod_fina",idw_fincon)
		idw_fincon.settransobject(SQLCA)
		idw_fincon.retrieve()
		idw_fincon.setfilter(fil_fina)
		idw_fincon.filter()
		tab_1.tp_1.dw_serv_cita.reset()
		dw_cita_asig.reset()
	...
	case 2		
		...
		if not isnull(i_dw_grupc.getitemstring(i_dw_grupc.getrow(),"codaadx")) then
			...
			dw_escog_profe.setitem(1,2,idw_fincon.getitemstring(idw_fincon.getrow(),'codfin'))
			accepttext()			
		else
			Modify("codproced.dddw.name='dw_proc_gcita'")
			tab_1.tp_1.dw_serv_cita.modify("cproced.dddw.name='dw_proc_gcita'")
			if isnull(getitemstring(1,1)) or isnull(getitemstring(1,2)) then
				filtro=''
			else
				filtro="'"+getitemstring(1,1)+"' and codfin ='"+getitemstring(1,2)
			end if
		end if
