$PBExportHeader$w_canasta_articulo.srw
forward
global type w_canasta_articulo from window
end type
type pb_cancel from picturebutton within w_canasta_articulo
end type
type pb_ok from picturebutton within w_canasta_articulo
end type
type pb_1 from picturebutton within w_canasta_articulo
end type
type pb_insert from picturebutton within w_canasta_articulo
end type
type st_1 from statictext within w_canasta_articulo
end type
type dw_escoger from datawindow within w_canasta_articulo
end type
type dw_getareo from datawindow within w_canasta_articulo
end type
type dw_descrip from datawindow within w_canasta_articulo
end type
end forward
...
global w_canasta_articulo w_canasta_articulo
...
//i_st.dw_canasta.setfilter('nservicio='+string(i_st.item))
i_st.dw_canasta.setfilter('nservicio='+string(i_st.nserv))
i_st.dw_canasta.filter()
long j
if i_st.dw_canasta.rowcount()>0 then
	dw_getareo.enabled=false
	for j=1 to i_st.dw_canasta.rowcount()
		dw_escoger.insertrow(j)
		dw_escoger.setitem(j,'cod_insumo',i_st.dw_canasta.getitemstring(j,'cod_insumo'))
		dw_escoger.setitem(j,'insumo',i_st.dw_canasta.getitemstring(j,'insumo'))
		dw_escoger.setitem(j,'nro_insumo',i_st.dw_canasta.getitemnumber(j,'nro_insumo'))
		dw_escoger.setitem(j,'umed',i_st.dw_canasta.getitemstring(j,'umed'))
		dw_escoger.setitem(j,'conversion',i_st.dw_canasta.getitemnumber(j,'conversion'))
		dw_escoger.setitem(j,'cantidad',i_st.dw_canasta.getitemnumber(j,'cantidad'))
		dw_escoger.setitem(j,'codgenerico',i_st.dw_canasta.getitemstring(j,'codgenerico'))
		dw_escoger.setitem(j,'descrip_generico',i_st.dw_canasta.getitemstring(j,'descrip_generico'))
		dw_escoger.setitem(j,'facturar',i_st.dw_canasta.getitemnumber(j,'facturar'))
		if i_st.dw_canasta.getitemnumber(j,'cantidad') > 0 then
			dw_escoger.setitem(j,'vprom',i_st.dw_canasta.getitemnumber(j,'vtotal')/i_st.dw_canasta.getitemnumber(j,'cantidad'))
		end if
		dw_escoger.setitem(j,'cod_arti',i_st.dw_canasta.getitemstring(j,'codarticulo'))
		if i_st.dw_canasta.getitemstatus(j,0,primary!)=DataModified! or i_st.dw_canasta.getitemstatus(j,0,primary!)=notmodified! then
			dw_escoger.setitem(j,'borrar','0')
		end if
		i_st.dw_lote.setfilter('nservicio_serv='+string(i_st.item)+' and nro_insumo_serv='+string(i_st.dw_canasta.getitemnumber(j,'nro_insumo')))
		i_st.dw_lote.filter()
		if i_st.dw_lote.rowcount()>0 then
			dw_escoger.setitem(j,'descrip_arti',i_st.dw_lote.getitemstring(1,'lote')+' - '+i_st.dw_canasta.getitemstring(j,'descripcion'))
			dw_escoger.setitem(j,'lote',i_st.dw_lote.getitemstring(1,'lote'))
			dw_escoger.setitem(j,'convenio',i_st.dw_lote.getitemstring(1,'convenio'))
			dw_escoger.setitem(j,'fecha_venc',i_st.dw_lote.getitemdatetime(1,'fecha_venc'))
		else
			dw_escoger.setitem(j,'descrip_arti',''+i_st.dw_canasta.getitemstring(j,'descripcion'))
		end if
		dw_escoger.setitem(j,'descrip_arti_ori',i_st.dw_canasta.getitemstring(j,'descripcion'))
		dw_escoger.setitemstatus(j,0,primary!,datamodified!)
		dw_escoger.setitemstatus(j,0,primary!,notmodified!)
	next
...
//		dw_escoger.retrieve(i_st.cproc,idw_getareo.getitemstring(1,'codgetareo'))//siempre va a se mayor a cero filas
//		for j=1 to dw_escoger.rowcount()
//			if idw_genericos.retrieve(i_st.cproc,dw_escoger.getitemstring(j,'cod_insumo'),dw_getareo.getitemstring(1,1))=0 then continue
//			dw_escoger.setitem(j,'codgenerico',idw_genericos.getitemstring(1,'c_medica'))
...
//			dw_escoger.setitem(j,'nro_insumo',j)
//			dw_escoger.setrow(j)
...
event close;i_st.dw_canasta.setfilter('')
i_st.dw_canasta.filter()
closewithreturn(this,ls_sale)
end event
...
for j=1 to dw_escoger.rowcount()
	if dw_escoger.getitemstring(j,'borrar')='0'  then continue //ya fue guardado y no estoy dejando modificar
	
	//borrar el anterior por si cambio algo del producto 
	nro_insum = dw_escoger.getitemnumber(j,'nro_insumo')
	donde = i_st.dw_canasta.find('nro_insumo='+string(nro_insum),1,i_st.dw_canasta.rowcount())
	if donde <> 0 then i_st.dw_canasta.deleterow(donde)
	i_st.dw_lote.setfilter('nservicio_serv='+string(i_st.nserv)+' and nro_insumo_serv='+string(nro_insum))
	i_st.dw_lote.filter()
	do while i_st.dw_lote.rowcount()>0
		i_st.dw_lote.deleterow(0)
	loop
//	if dw_escoger.getitemNumber(j,'cantidad') = 0 then Continue
	//fin borrar el anterior por si cambio algo del producto
	
	i_st.dw_canasta.insertrow(j)
	i_st.dw_canasta.setitem(j,'contador',i_st.contador)
	i_st.dw_canasta.setitem(j,'clugar',i_st.clugar)
	i_st.dw_canasta.setitem(j,'nservicio',i_st.nserv)
	i_st.dw_canasta.setitem(j,'nro_insumo',dw_escoger.getitemnumber(j,'nro_insumo'))
	...
	if dw_escoger.getitemstring(j,'lote')<>'' and not isnull(dw_escoger.getitemstring(j,'lote')) then
		...
		//i_st.dw_lote.setitem(donde,'fecha_venc',datetime(date(left(dw_escoger.getitemstring(j,'fecha_venc'),8)),time(mid(dw_escoger.getitemstring(j,'fecha_venc'),10))))
		...
		i_st.dw_lote.setitem(donde,'nservicio_serv',i_st.nserv)
		i_st.dw_lote.setitem(donde,'nro_insumo_serv',dw_escoger.getitemnumber(j,'nro_insumo'))
	end if
next
ls_sale='ok'
close(parent)
...
alignment htextalign = left!
string powertiptext = "Insertar Insumo"
end type

event clicked;long j,donde
donde=dw_escoger.insertrow(0)
for j=dw_escoger.rowcount() to 1 step -1
	if dw_escoger.find('nro_insumo='+string(j),1,dw_escoger.rowcount())=0 then exit
next
dw_escoger.setitem(donde,'nro_insumo',j)
i_cambio=true
end event
...
integer taborder = 30
string title = "none"
string dataobject = "dw_escoge_generico_articulo"
...
//		donde=idw_genericos.find('medicamento="'+data+'"',1,idw_genericos.rowcount())
		donde=idw_genericos.find('descrip_generico="'+data+'"',1,idw_genericos.rowcount())
		...
		if idw_articulo.retrieve(idw_genericos.getitemstring( donde, 'codgenerico'),i_st.alm,getitemnumber(row,'cantidad'))>0 then
...
//	dw_escoger.setitem(j,'codgenerico',idw_genericos.getitemstring(1,'c_medica'))
	...
	dw_escoger.setitem(j,'descrip_generico',idw_genericos.getitemstring(1,'descrip_generico'))
	dw_escoger.setitem(j,'cantidad',idw_genericos.getitemnumber(1,'cantidad'))
	dw_escoger.setitem(j,'umed',idw_genericos.getitemstring(1,'umed'))
	dw_escoger.setitem(j,'conversion',idw_genericos.getitemnumber(1,'conversion'))
	dw_escoger.setitem(j,'nro_insumo',j)
	dw_escoger.setrow(j)
	dw_escoger.event itemchanged(j,dw_escoger.object.descrip_generico,idw_genericos.getitemstring(1,'descrip_generico'))
next
