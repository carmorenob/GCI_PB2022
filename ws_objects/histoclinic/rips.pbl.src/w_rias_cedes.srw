$PBExportHeader$w_rias_cedes.srw
forward
global type w_rias_cedes from window
end type
type gb_4 from groupbox within w_rias_cedes
end type
type gb_1 from groupbox within w_rias_cedes
end type
type dw_getar from datawindow within w_rias_cedes
end type
type dw_temp from datawindow within w_rias_cedes
end type
type dw_det_lote from datawindow within w_rias_cedes
end type
type dw_export from datawindow within w_rias_cedes
end type
type dw_trae from datawindow within w_rias_cedes
end type
type tab_2 from tab within w_rias_cedes
end type
...
type rb_names from radiobutton within tp2_1
end type
...
type dw_lug from datawindow within w_rias_cedes
end type
type rb_8 from radiobutton within w_rias_cedes
end type
type rb_7 from radiobutton within w_rias_cedes
end type
type rb_10 from radiobutton within w_rias_cedes
end type
type rb_9 from radiobutton within w_rias_cedes
end type
type dw_tipin from datawindow within w_rias_cedes
end type
end forward
...
string title = "Generador de RIPS Cedes"
...
global w_rias_cedes w_rias_cedes

type variables
datawindowchild idw_contrato,idw_emp,gc_regimen
long i_NRO_DIG,i_numarc,l_dec_fact
string anterior,orden,CONCATENA_FAC,i_decual,l_capiv='0',ls_rips_cd
end variables
...
public function long lf_trae_ndoc (string p_decual, string p_cdoc)
...
public function long lf_trae_ndoc (string p_decual, string p_cdoc);long ndoc,n_ini,n_fin
string err,por_lug,p_desdoc='Radicaciones',p_clug
p_clug=clugar
...
public function long f_rips (boolean p_faltan, long p_nradica, string p_clug_rad, datawindow p_dw_conts, string p_decual, boolean p_cups, long p_cero, string p_por_lugar, string p_reserva, string p_tipo_rad);string cual,param,nom_arch,emp,docname,realn[12],formato,log_txt ,cips,tipo_rad,l_cont
//realn:real_name
long j,i,filas,arch,num_af,num_ad,num_us,n_reg[7],num_radica=0,factus,p_at=0
...
if p_faltan then
	...
	if p_nradica=0 then
		...
		INSERT INTO ripsradica ( num_radicacion,clugar,tipo, cemp , fecha_radica, fecha_inicial, fecha_final ,usuario,valor_capita,valor_evento,nro_factu,por_lugar,reserva) 
		values (:num_radica,:clugar,:tipo_rad,:emp,:fecha_radica,:fecha_ini,:fecha_fin,:usuario,:tot_capita,:tot_evento,:factus,:p_por_lugar,:p_reserva);
		if sqlca.sqlcode=-1 then
			if p_decual='lote' then //escribir en el log
...
if dw_trae.rowcount()>=0 and (p_reserva='0' or p_reserva='1') then 
	if p_dw_conts.getitemnumber(1,'forecat')=0 or isnull(p_dw_conts.getitemnumber(1,'forecat')) then //son los rips normales
		for arch=1 to 7
			...
			choose case cual
				case "Consulta"  //archivo/empresa/contrato/fechas
					...
					nom_arch="AC"
				case "Procedimientos"
					dw_trae.dataobject="dr_rips_procedimientos_viejo_cedes"
				...
				case "Medicamentos"
					...
					nom_arch="AM"
				case "Otros"
					dw_trae.dataobject="dr_rips_otros_viejo_cedes"
			...
			dw_trae.reset()
			if dw_trae.retrieve(p_nradica,p_clug_rad,p_por_lugar,cser,p_tipo_rad,l_capiv)<>-1 then
				dw_export.reset()
				if dw_trae.rowcount()=0 then
					if p_decual='emp' then
					...
					n_reg[arch]=dw_trae.rowcount()
					 ...
					 if cual='Consulta' or cual='Procedimientos' or cual='Medicamentos' or cual='Otros' then
					 	dw_trae.setfilter('vproced>0')
					else
						dw_trae.setfilter('')
					end if
					dw_trae.filter()
					...
					for j=1 to filas
						...
						choose case cual
							...
							case "Medicamentos"
								...
								dw_export.setitem(i,'c_ips',dw_trae.getitemstring(j,'este_codigo'))//cips
								...
								dw_export.setitem(i,'c_medica',dw_trae.getitemstring(j,"cum_espe"))//cmedica
					...
					next
					nom_arch+=string(p_nradica)+'_'+p_tipo_rad
					docname=nom_arch
		...
		dw_trae.dataobject="dr_rips_af_transacc_viejo_cedes"
		dw_trae.settransobject(sqlca)
		dw_trae.reset()
		...
		for j=1 to filas
			...
			If  p_por_lugar='0' then
				...
				dw_export.setitem(i,3,dw_trae.getitemstring(j,"td_ips"))
				if tab_2.tp2_1.cbx_6.checked then 
					dw_export.setitem(i,4,dw_trae.getitemstring(j,"doc_ips")+'-'+dw_trae.getitemstring(j,"ips_dv"))
				else
					dw_export.setitem(i,4,dw_trae.getitemstring(j,"doc_ips"))
				end if
			else
				dw_export.setitem(i,2,left(dw_trae.getitemstring(j,"ips_lugar"),60))	
				dw_export.setitem(i,3,dw_trae.getitemstring(j,"tipo_doc"))
				if tab_2.tp2_1.cbx_6.checked then 
					dw_export.setitem(i,4,dw_trae.getitemstring(j,"documento")+'-'+dw_trae.getitemstring(j,"l_dv"))
				else
			...
			if concatena_fac="1" then
			    if p_dw_conts.getitemstring(1,'ctipo')='1' then  
					if  p_por_lugar<>'1' then
						 if not isnull(dw_trae.getitemstring(j,"prefijo")) then 
							dw_export.setitem(i,5,dw_trae.getitemstring(j,"prefijo")+string(dw_trae.getitemnumber(j,"nfact"),formato)) //factura
						else
					...
					dw_export.setitem(i,5,string(p_nradica,formato))
				end if				
			else
			...
			if isnull(dw_trae.getitemdatetime(j,"fechaingreso")) then
				fecha_ini=dw_trae.getitemdatetime(j,"fecha")
				fecha_fin=dw_trae.getitemdatetime(j,"fecha")
			else
				fecha_ini=dw_trae.getitemdatetime(j,"fechaingreso")
				fecha_fin=dw_trae.getitemdatetime(j,"fechaegreso")
			end if
		...
		nom_arch="AF"+string(p_nradica)+'_'+p_tipo_rad
		docname=nom_arch
		...
		nom_arch="US"+string(p_nradica)+'_'+p_tipo_rad
		docname=nom_arch
		dw_export.reset()
		filas=dw_trae.rowcount()
		for j=1 to filas
			...
			if isnull(dw_trae.getitemstring(j,'pais')) or dw_trae.getitemstring(j,'pais')='' then 	
				dw_export.setitem(i,'doc',dw_trae.getitemstring(j,"documento"))
			else
		...
		nom_arch="CT"+string(p_nradica)+'_'+p_tipo_rad
		docname=nom_arch
		if dw_export.rowcount()>0 then
			if p_decual='emp' then
				if tab_2.tp2_1.rb_names.checked then//preguntar una a uno
		...
		if p_decual='lote' and p_nradica>0  then
			log_txt='NRO RADICACIÓN: '+string(p_nradica)+'_'+p_tipo_rad+'~r~n'
			filewrite(i_numarc,log_txt)
		end if
		////// 1
	...
	if p_reserva='1' and p_nradica<>0 then
		...
		UPDATE ripsradica SET reserva = '0', valor_evento=:tot_evento,nro_factu=:factus,RESERVA_CAP=RESERVA_CAP+1
...
if g_motor='postgres' then
	dw_trae.dataobject="dr_furips_01_postgres"
else
...
if dw_trae.retrieve(p_nrad,p_clug_rad,p_tipo_rad)>0 then
	...
	for j=1 to dw_trae.rowcount()
		...
		if concatena_fac="1" then
			 if not isnull(dw_trae.getitemstring(j,"prefijo_ips")) then 
				dw_export.setitem(i,'factura',dw_trae.getitemstring(j,'prefijo_ips')+dw_trae.getitemstring(j,'nfact') )
			end if
			 if not isnull(dw_trae.getitemstring(j,"prefijo_lugar")) then 
				dw_export.setitem(i,'factura',dw_trae.getitemstring(j,'prefijo_lugar')+dw_trae.getitemstring(j,'nfact') )
			else
		...
		dw_export.setitem(i,'depto_pro',dw_trae.getitemstring(j,'depto_pro'))
	...
	if tab_2.tp2_1.rb_names.checked then//preguntar una a uno
		//docname es el largo y nom_arc el solo archivito
		if GetFileSaveName("Archivo Furips1 ("+nom_arch+")",docname, nom_arch, "txt", "Archivos de Texto (*.txt),*.txt" )<>1 then
			if p_faltan then messagebox("Atención",'Aún así se generó la radicación Nro:'+string(p_nrad))
			return 0
		end if
	else//mandarlos todos al directorio
	...
	if tab_2.tp2_1.rb_names.checked then//preguntar una a uno
		//docname es el largo y nom_arc el solo archivito
		if GetFileSaveName("Archivo Furips2 ("+nom_arch+")",docname, nom_arch, "txt", "Archivos de Texto (*.txt),*.txt" )<>1 then
			if p_faltan then messagebox("Atención",'Aún así se generó la radicación Nro:'+string(p_nrad))
			return 0
		end if
	else//mandarlos todos al directorio
	...
	if dw_trae.retrieve(p_nrad,p_clug_rad,p_tipo_rad)>0 then
		...
		for j=1 to dw_trae.rowcount()
			...
			if  dw_trae.getitemstring(j,'furips')='5' then
				dw_export.setitem(i,'descripcion',left(f_quita_basura(dw_trae.getitemstring(j,'descrip'),0),40))
			else
...
event open;SELECT cadena into :CONCATENA_FAC
FROM parametros_gen
...
choose case colum
	...
	case 62,63,64,65,66
		...
		setnull(nulo)
		if data<>"" then
			st=f_check_diag(data,getitemstring(row,"sexo"),getitemnumber(row,"dias"),este,'0',this.getitemstring(row,'rips'),'0')
...
if dataobject="dr_rips_rn" or dataobject="dr_rips_rn_viejo" then
	...
	case 15,16
		if getitemstring(row,colum)<>"" then
			...
			st=f_check_diag(getitemstring(row,colum),getitemstring(row,"sexorn"),1,este,'0',this.getitemstring(row,'rips'),'0')
			if st.descrip_diag="" then
				setitem(row,colum,"")
				setitem(row,left(col,len(col)-1),nulo)
				return 1
...
choose case colum
	...
	case 62,63,64,65,66
		if getitemstring(row,colum)<>"" then
			este=getitemstring(row,left(col,len(col)-1))
			st=f_check_diag(getitemstring(row,colum),getitemstring(row,"sexo"),getitemnumber(row,"dias"),este,'0',this.getitemstring(row,'rips'),'0')
			if st.descrip_diag="" then
				setitem(row,colum,"")
				setitem(row,left(col,len(col)-1),nulo)
				return 1
...
choose case cual
	case "Consulta"  //archivo/empresa/contrato/fechas
		if tab_2.tp2_1.tab_1.selectedtab=1 then
			...
			dw_ria.settransobject(sqlca)
			dw_ria.retrieve(tab_2.tp2_1.tab_1.tp_p.dw_radica.getitemnumber(tab_2.tp2_1.tab_1.tp_p.dw_radica.getrow(),"num_radicacion"),tab_2.tp2_1.tab_1.tp_p.dw_radica.getitemstring(tab_2.tp2_1.tab_1.tp_p.dw_radica.getrow(),"clugar"),'%','%',tab_2.tp2_1.tab_1.tp_p.dw_radica.getitemstring(tab_2.tp2_1.tab_1.tp_p.dw_radica.getrow(),"tipo"),l_capiv)
			return
		end if
...
long textcolor = 33554432
long backcolor = 67108864
string text = "Confirmar Nombres"
boolean checked = true
end type

event clicked;sle_dir.enabled=false
...
boolean bringtotop = true
string title = "none"
string dataobject = "dw_empresa_contrato_cedes"
boolean border = false
boolean livescroll = true
end type
...
if this.getcolumnname() = "ctreg" then
	...
	idw_contrato.retrieve(codem,regim)	
end if
...
boolean bringtotop = true
string title = "none"
string dataobject = "dw_radica_rips_cedes"
boolean hscrollbar = true
boolean vscrollbar = true
boolean hsplitscroll = true
boolean livescroll = true
...
tab_2.tp2_1.tab_1.tp_p.dw_det.retrieve(getitemnumber(getrow(),'num_radicacion'),getitemstring(getrow(),'clugar'),getitemstring(getrow(),'tipo'))
tab_2.tp2_1.t1.tp2.dw_act_final.retrieve(getitemnumber(getrow(),'num_radicacion'),getitemstring(getrow(),'clugar'),getitemstring(getrow(),'tipo'))
...
if dw_radica.rowcount()>0 then
	if not isnull(dw_radica.getitemstring(dw_radica.getrow(),"estado")) then
		...
		if dw_radica.getitemstring(dw_radica.getrow(),"reserva")='0' then
			nrad=f_rips(false,dw_radica.getitemnumber(dw_radica.getrow(),"num_radicacion"),dw_radica.getitemstring(dw_radica.getrow(),"clugar"),dw_det,'emp',tab_2.tp2_1.rb_4.checked,-1,dw_radica.getitemstring(dw_radica.getrow(),"por_lugar"),dw_radica.getitemstring(dw_radica.getrow(),"reserva"),dw_radica.getitemstring(dw_radica.getrow(),"tipo"))
		else
			nrad=f_rips(true,dw_radica.getitemnumber(dw_radica.getrow(),"num_radicacion"),dw_radica.getitemstring(dw_radica.getrow(),"clugar"),dw_det,'emp',tab_2.tp2_1.rb_4.checked,-1,dw_radica.getitemstring(dw_radica.getrow(),"por_lugar"),dw_radica.getitemstring(dw_radica.getrow(),"reserva"),dw_radica.getitemstring(dw_radica.getrow(),"tipo"))
		end if
		choose case nrad
			case -1,-10
				rollback;
